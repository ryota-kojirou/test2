            ...
            for hum in np.sort(humans_id):
                ...
                for c in range(-3, 0):
                  for i in range(2):
                    ...
                    hand_y = []
                    for n in range(3):
                      if len(df_human[:, (i*3+n+2) * 3 + 1]) > 5:
                        x = df_human[:, (i*3+n+2) * 3 + 1][c*2] * w_pxl
                        y = df_human[:, (i*3+n+2) * 3 + 2][c*2] * h_pxl
                        hand_x.append(x)
                        hand_y.append(y)
                    plt.plot(hand_x, hand_y, linewidth=400/fig_resize, alpha=(1+c/5), color = 'forestgreen')
